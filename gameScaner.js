// USER ACTIVITY: set up timer
let timeLeft = 30;

// USER ACTIVITY: set up score
let score = 0;

// USER ACTIVITY: check if game is paused
let isPaused = false;

// key that tracks which burrow the mole is currently in
...
class GameScene extends Phaser.Scene {
  ...
  create() {
    // USER ACTIVITY: Create function that updates the timer by the amount of time that has elapsed
  ...
  update() {
    ...
    // USER ACTIVITY: Create function that updates user score by amount of time that has elapsed
    ...
    const applyHitReward = () => {
      ...
      // USER ACTIVITY: Update the score
      updateScore(5);
    ...
    // USER ACTIVITY: Pause game if game is not yet paused, and unpause game if it is already paused
    const togglePause = () => {
      if (isPaused === true) {
        isPaused = false;
    ...
    if (Phaser.Input.Keyboard.JustDown(gameState.spaceKey)) {
      // USER ACTIVITY: When user hits the space key, flip the pause state to the opposite of whatever it currently is
      togglePause();
    }
  }
